...
def main():
  ...
  try:
    ...
    x1 = 0.1 #float(input("Enter X offset (m): "))
    y1 = 0#float(input("Enter Y offset (m): "))
    z1 = -0.34 #float(input("Enter Z offset (m): "))
  except ValueError:
  ...
  joint_5_angles = np.arange(-np.pi/8, np.pi/8 + np.pi/16, np.pi/16)
    ...
    # Main iteration loop
  with open("new_transformations.csv", mode="a", newline="") as file:
              ...
              for q5 in joint_5_angles:
                for q6 in joint_6_angles:
                  ...
                  if (img is not None) and (img.size > 0):
                    ...
                    cv2.imwrite(
                        f"new_imag/X{x2}Y{y2}Z{z2}_J5{q5}_J6{q6}.png", img
                    )
                    T_bg = robot.fk(robot.get_q())
                    flattened_T_bg = T_bg.flatten()
                    writer.writerow([x2, y2, z2, q5, q6, *flattened_T_bg])
              robot.move_to_q(q_current)
            else:
